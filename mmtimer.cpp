...
double TimerGetTime()
{
	...
	if (g_timer.performance_timer)
	{
		...
		return (((double)(time - g_timer.performance_timer_start) * g_timer.resolution));
	}
	else
	{
		// Return The Current Time Minus The Start Time Multiplied By The Resolution 
		return (((double)(timeGetTime() - g_timer.mm_timer_start) * g_timer.resolution));
	}
}

double TimerGetTimeMS()
{
